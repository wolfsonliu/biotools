...
organismList <- function(origin   = FALSE)
...
pathwayInfo <- function(pathway.id)
...
pathwayEnrichment <- function(gene.list,
...
###{{{
    {
        if (!is.atomic(gene.list)) {
            gene.list <- as.vector(gene.list)
        }
        if (!exists("pathway.list")) {
            pathway.list <<- pathwayList(organism)
        }
        if (!exists("pathwayEnrichment.pathway.info")) {
            pathwayEnrichment.pathway.info <<- list()
            message("Fetching pathway information...")
            for (id.path in pathway.list$ID) {
                pathwayEnrichment.pathway.info[[id.path]] <<- pathwayInfo(id.path)
            }
        }
        ## wa: white all
        ## wd: white drawn
        ...
        pathway.result <- data.frame(
            ...
            Type              = "KEGG pathway",
            ID                = names(pathwayEnrichment.pathway.info),
